...
int main(int argc, char **argv, char **env){
    ...
    for(simcyc = 0; simcyc < MAX_SIM_CYC; simcyc++) {
        ...
        vbdPlot(int(top->dout1), 0, 255);   // plot the first sine wave
        vbdPlot(int(top->dout2), 0, 255);   // plot the second sine wave
        vbdCycle(simcyc + 1);       // reporting the cycle count on bottom right of the display

        // changing the initial stimuli
        top->phase = vbdValue();     // value of the incr is determined by the value shown on the display, to change with the rotary switch
        top->rst = (simcyc < 2);
        top->en = (simcyc > 3);
